...
- The same endpoint now supports `mode="search"` with one combined tone-word query. Search maps shared picker words into sparse taxonomy-v2 dimensions, performs typed direct plus target-conditioned source retrieval bounded to three anchors per type, predicts transient combo tones, reranks by exact masked distance, and then hands the result to normal walking. A `serene + loving` production smoke returned search metadata and a subsequent same-video/new-audio walk; post-deploy warm searches completed in 1.2-1.5 seconds.
- The adaptive 54-word picker hierarchy and suggestion algorithm now live in `tone-core`, with reusable React state/UI in `packages/shared`. Media Manager `/combos` has a locally built controlled explorer with initial sound enabled, review-aligned top suggestions and bottom selected-word submission, initial random playback, tone search, manual/automatic walking, bounded history, diagnostics, and review deep links; this web UI is not published from this repository.
- Darenkeck `/` now has a locally built curiosity-gated tone explorer. First use shows a versioned local explainer, selected words start global search and bounded automatic walking, and empty submission starts a fully random sequence. The explorer reuses the shared picker/submit pile, temporarily collapses the bulletin, and remains unpublished pending browser and staging validation.
- Darenkeck `App` now persists across `/` and `/dev`: the same media elements, audio state, picker selections, and walk history survive SPA navigation while playback keeps advancing behind the translucent resume. Direct `/dev` starts a muted random combination; internal `/dev?print=1` suppresses all media for deterministic PDF generation.
