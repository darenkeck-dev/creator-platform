...
- Darenkeck has locally built persistent blog routes. The homepage exposes Resume and Blog controls; navigating through the blog index and entries retains the same playback elements. Below the desktop breakpoint, document cards fill the viewport width and mute/tone controls move from their floating homepage positions to opposite sides of the sticky breadcrumbs. A preparation step validates post frontmatter, excludes explicit and filename-marked drafts from the generated bundle, and sorts published posts newest-first.
