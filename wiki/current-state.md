...
- Media Manager owns official music tracks/releases, ordered membership, cover assets, purchase links, readiness, and publication. Authenticated catalog APIs and `GET /public/music` are deployed; Library includes the release index and creation action, while `/releases/new` and `/releases/:id` provide creation and editing. Darenkeck now has a source-complete `/music` catalog and persistent full-track mode: tracks use their official HLS audio as timeline authority, pair with a fresh random looping video, auto-advance within a release, retain controls across routes, and return to the preserved ambient assignment after the final track. The releases remain drafts and this Darenkeck update is not deployed.
...
- Darenkeck `/` now has a deployed curiosity-gated tone explorer. First use shows a versioned local explainer, selected words start global search and bounded automatic walking, and empty submission starts a fully random sequence. The explorer uses a full-screen translucent backdrop, locks and restores page scrolling, places suggestions around one-third of the viewport height, keeps selected words at the bottom, reuses the shared picker/submit pile, and temporarily hides the homepage panel without changing its explicit minimized state. Submit shows a success check for one second before closing the overlay. On compact document routes, opening tone selection hides the embedded mute control, leaves the bar's tone icon as a launcher, and exposes a separate overlay close control. Production browser and search-to-walk smokes passed; broader target browser-matrix validation remains follow-up work.
- Darenkeck has deployed persistent blog routes. The homepage exposes Resume and Blog controls; navigating through the blog index and entries retains the same playback elements. Below the desktop breakpoint, document cards fill the viewport width and mute/tone controls move from their floating homepage positions to opposite sides of the sticky breadcrumbs. A preparation step validates post frontmatter, excludes explicit and filename-marked drafts from the generated bundle, and sorts published posts newest-first.
- Darenkeck bulletin preparation validates required title, date, summary, and Markdown body fields; excludes drafts; carries optional featured-image metadata; and emits a newest-first manifest. The homepage renders the latest three published items and links to News from its footer rail, while archive and detail routes preserve playback continuity.
- The homepage and every persistent document route share one minimize mode. Document minus controls live in the sticky breadcrumb bar without adding vertical chrome; minimized content remains mounted, compact media controls return to fixed positions, and the fixed `Daren Keck [+]` control restores the current route and its captured scroll offset.
