...
- The adaptive 54-word picker hierarchy and suggestion algorithm now live in `tone-core`, with reusable React state/UI in `packages/shared`. Media Manager `/combos` has a locally built controlled explorer with initial random playback, tone search, manual/automatic walking, bounded history, diagnostics, and review deep links; this web UI is not published from this repository.
