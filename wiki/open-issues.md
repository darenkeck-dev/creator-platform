...
- Replace random-only continuation with the gated predicted-combo-tone walk while retaining random fallback.
...
- Extend provider-neutral vector queries with audio/video metadata filters and validate AWS filter behavior.
- Implement `combo-tone-predictor/v0`, exact predicted-tone distance, top-five weighted sampling, and bounded history as pure tested logic.
- Add the isolated read-only `POST /public/combos/select` Lambda, then enable Darenkeck walk mode behind a controlled flag.
