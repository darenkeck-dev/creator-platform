...
- `DocumentShell` keeps Home, a section-root link, and docked Tone in a 40px sticky row at the top of the document shelf, then portals playback, route actions, and the size control into a separate three-column, 40%-black blurred dock flush to the viewport base. Ambient, document, loading, and dedicated music docks all use equal fixed-width outer columns so center metadata stays geometrically centered; their rightmost slot always contains expanded `-` or minimized `+`, with music Stop immediately to its left. Docks, gradients, progress rails, and seek targets are centered at the same `max-w-4xl` width as the content shelf; they remain full-width below that maximum. The dock's center renders ambient released-track metadata supplied by `App`. The square, borderless upper row uses a translucent section primary color and an SVG mask to cut out its uppercase name at the shared diagonal positions: News 10%, Music 35%, Blog 56%, and Resume 87%. An 8px transparent gap separates it from the independently backed document surface. Breadcrumbs and the favicon are omitted. When sticky, the row increases its color opacity and backdrop blur, and Tone becomes a compact 32px control without its circular background, shadow, or local blur. The floating signature and Tone hide.
- Home renders its primary navigation as four absolutely positioned SVG-backed rows immediately above the shelf surface. Each route color fills an SVG rectangle whose text mask cuts transparent letters through it. The hamburger raises or lowers the full stack from behind the shelf without changing layout or viewport height.
- Every document route exposes the same white general-navigation hamburger at the active row's far right. Opening preserves the canonical Resume/Blog/Music/News stack: earlier sections appear above the active row and later sections below it. Undocked expansion runs in normal flow and moves the content surface down; once sticky, the container remains 40px and the expanded stack overlays content without shifting it. Only inactive rows receive the Home right-to-left clip reveal. Docked expansion applies 16px backdrop blur plus reduced brightness and saturation at each clipped inactive-row group, ensuring every row obscures underlying text inside its letter cutouts. When sticky, Tone occupies a fixed 32px slot immediately left of the hamburger.
