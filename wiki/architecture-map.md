...
- `DocumentShell` keeps Home, a section-root link, and expanded `-` in a 40px sticky row at the top of the document shelf, then portals playback and route actions into a separate three-column, 40%-black blurred dock flush to the viewport base. Ambient, document, loading, and dedicated music docks all use equal fixed-width outer columns so center metadata stays geometrically centered before and after Stop or size controls appear. Docks, gradients, progress rails, and seek targets are centered at the same `max-w-4xl` width as the content shelf; they remain full-width below that maximum. The dock's center renders ambient released-track metadata supplied by `App`. The square, borderless upper row uses a translucent section primary color and an SVG mask to cut out its uppercase name at the shared diagonal positions: News 10%, Music 35%, Blog 56%, and Resume 87%. An 8px transparent gap separates it from the independently backed document surface. Breadcrumbs and the favicon are omitted. When sticky, the row increases its color opacity and backdrop blur, and Tone moves immediately before `-` as a compact 32px control without its circular background, shadow, or local blur. The floating signature and Tone hide. Minimized states expose `+` in the base row.
