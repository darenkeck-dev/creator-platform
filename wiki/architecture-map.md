...
- `DocumentShell` keeps Home, a section-root link, and expanded `-` in a 40px sticky row at the top of the document shelf, then portals playback and route actions into a separate 40%-black blurred dock flush to the viewport base. The square, borderless upper row uses a translucent section primary color and an SVG mask to cut out its uppercase name at the shared diagonal positions: News 10%, Music 35%, Blog 56%, and Resume 87%. An 8px transparent gap separates it from the independently backed document surface. Breadcrumbs and the favicon are omitted. When sticky, the row increases its color opacity and backdrop blur, and Tone moves immediately before `-` as a compact 32px control without its circular background, shadow, or local blur. The floating signature and Tone hide. This frees the bottom center for track metadata. Minimized states expose `+` in the base row.
