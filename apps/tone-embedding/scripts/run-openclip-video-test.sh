...
DEFAULT_OUTPUT_PATH="${APP_DIR}/tests/output/asset-tones-openclip-video.jsonl"
HOST_OUTPUT_PATH="${1:-${DEFAULT_OUTPUT_PATH}}"
IMAGE_NAME="${OPENCLIP_VIDEO_TEST_IMAGE:-tone-embedding-openclip-video-test:local}"
DOCKERFILE="${APP_DIR}/Dockerfile.openclip-video-test"
...
log "Running OpenCLIP video tone extraction in Docker"
log "Host output JSONL: ${HOST_OUTPUT_PATH}"
log "Docker image: ${IMAGE_NAME}"

if [[ "${REBUILD_OPENCLIP_VIDEO_TEST_IMAGE:-0}" == "1" ]] || ! docker image inspect "${IMAGE_NAME}" >/dev/null 2>&1; then
  log "Building Docker image with OpenCLIP preinstalled"
...
log "OpenCLIP video tone extraction complete"
